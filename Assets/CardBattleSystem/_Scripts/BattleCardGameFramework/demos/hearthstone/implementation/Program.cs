...
namespace hearthstone
{
    class MainClass
    {
        ...
        public static void Main(string[] args)
        {
            ...
            HearthstoneAI ai = new HearthstoneAI();

            string info = string.Empty;
            string input = string.Empty;
            do
            {
                if (game.State.ActivePlayers.Any(p => p.IsAI))
                {
                    Console.WriteLine("AI is thinking...");
                    ai.Action(game);
                    info = "AI performed actions.";
                    continue;
                }
        ...
        private static HearthstoneGame CreateGame()
        {
            ...
            // Create 4 players for 2 teams (2 players per team)
            for (int i = 0; i < 4; ++i)
            {
                int teamId = i / 2;
                bool isAI = (teamId == 1); // Team 1 is AI

                HearthstonePlayer player = new HearthstonePlayer();
                player.TeamId = teamId;
                player.IsAI = isAI;

                player.AddStat(StatKeys.Life, new Stat(30, 30));
                player.AddStat(StatKeys.Mana, new Stat(0, 10));
